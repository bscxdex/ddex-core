...
contract DDXRouter is IUniswapV2Router02,Ownable {
    ...
    function setSwapMing(address _swapMining) public onlyOwner{
    ...
    function getBNBReservers( address token) public view returns (uint reserveA, uint reserveB){
        return UniswapV2Library.getReserves(factory, token, WBNB);
    }


    function getBNBPair( address token) public view returns (address pair){
        return UniswapV2Library.pairFor(factory, token, WBNB);
    }
