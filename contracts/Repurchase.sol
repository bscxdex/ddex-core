...
contract Repurchase is Ownable {
    ...
    address public constant DDX = 0xcBD6Cb9243d8e3381Fea611EF023e17D1B7AeDF0;
    address public constant DDX_USDT = 0x8611a52e8AC5E10651DF7C4b58F42536f0bd2e7E;
    ...
    function swapV() external view returns (uint256 amountOut){
        ...
        uint256 amountHalf = amountIn.div(2);
        {
            ...
            uint256 amountInWithFee = amountHalf.mul(998);
            amountOut = amountHalf.mul(998).mul(reserve0) / reserve1.mul(1000).add(amountInWithFee);
    ...
    function swap() external onlyCaller returns (uint256 amountOut){
        ...
        uint256 amountHalf = amountIn.div(2);
        {
            (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(DDX_USDT).getReserves();
            uint256 amountInWithFee = amountHalf.mul(998);
            amountOut = amountHalf.mul(998).mul(reserve1) / reserve0.mul(1000).add(amountInWithFee);
            IERC20(USDT).safeTransfer(DDX_USDT, amountHalf);
